...
def collision(player_pos, enemy_pos):
    return not (player_pos[0] + 56 < enemy_pos[0] or player_pos[0] > enemy_pos[0] + 56 or player_pos[1] > enemy_pos[
        1] + 122 or player_pos[1] + 122 < enemy_pos[1])


class Simulate:

    def __init__(self, score=0, alive=True, observations=None, time=0, numSpawns=1, spawnTimer=5.0, enemies=None):
        self.score = score
        self.alive = alive
        self.time = time
        ...
        class EnemyOne:
            def __init__(self, pos, screen, enemyImg):
                self.pos = pos
        ...
        class Car:
            def __init__(self, pos, y_change, x_change, alive, screen, carImg):
                self.pos = pos
                self.carImg = carImg
                self.screen = screen
            ...
            def move(self):
                if self.alive:
                    ...
                    self.rect.center = self.pos
        if enemies is None:
            enemies = [EnemyOne((-10000, -10000), self.screen, self.enemyImg)]
        if observations is None:
            observations = [100.00, 10000.00, 10000.00, 10000.00, 10000.00]

        self.enemies = enemies
        self.observations = observations
        self.player = Car((0, 400), 0, 0, self.alive, self.screen, self.carImg)

    def run(self, input_net, width=500, height=500):
        class EnemyOne:
            def __init__(self, pos, screen, enemyImg, numSpawns):
                self.pos = pos
                self.enemyImg = enemyImg
                self.screen = screen
                super(EnemyOne, self).__init__()
        ...
        if self.alive:
            ...
            x_change_outer = 0
            y_change_outer = 0
            spawnPos = [width / 2 + 95, width / 2 + 19, width / 2 - 75, width / 2 - 140]
            if input_net == "a":
                x_change_outer = 0.4
            elif input_net == "d":
                x_change_outer = -0.4
            ...
            if self.spawnTimer >= 5.0:
                ...
                position = spawnPos.index(spawnPosOne)
                if position == 0:
                    spawnPos.remove(spawnPos[1])
                elif position == 3:
                    spawnPos.remove(spawnPos[2])
                else:
                    spawnPos.remove(spawnPos[position+1])
                    spawnPos.remove(spawnPos[position-1])
                self.enemies.append(EnemyOne((spawnPosOne, 0), self.screen, self.enemyImg, self.numSpawns))
                ...
                if self.numSpawns > 5 and random.choice((True, False)):
                    ...
                    self.enemies.append(EnemyOne((spawnPosTwo, 0), self.screen, self.enemyImg, self.numSpawns))
                    """if self.numSpawns > 40 and random.choice((True, False)):
                        spawnPosThree = random.choice(spawnPos)
                        spawnPos.remove(spawnPosThree)
                        self.enemies.append(EnemyOne((spawnPosThree, 0), self.screen, self.enemyImg, self.numSpawns))"""
            self.observations = []
            ...
            self.observations.append(abs(saved_y))
            if abs(saved_x) > 75.0:
                saved_x = 75.0
